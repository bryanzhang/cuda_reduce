...
__global__ void reduceNeighbor(int* output, int* input) {
        ...
        for (int stride = 1; stride < blockDim.x; stride <<= 1) {
                if ((global_tid % (2 * stride)) == 0) {
                        input[global_tid] += input[global_tid + stride];
                }
                __syncthreads();
        }
        if (threadIdx.x == 0) {
                output[blockIdx.x] = input[global_tid];
        }
}

static const int narr = (1 << 24);
static const int numThreads = 1024;
static const int numBlocks = (narr + numThreads - 1) / numThreads;
