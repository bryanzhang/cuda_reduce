...
__global__ void reduceNeighbor(int* output, int* input) {
        int tid = threadIdx.x;
        int* inner_input = input + 4 * blockIdx.x * blockDim.x;
        int stride = (blockDim.x << 1);
        inner_input[tid] += inner_input[tid + stride];
        inner_input[tid] += inner_input[tid + stride + blockDim.x];
...
static const int numBlocks = (narr + numThreads - 1) / numThreads / 4;
